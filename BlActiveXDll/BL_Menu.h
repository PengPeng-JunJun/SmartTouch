...
public:
	...
	void LoadDefaultPath(void)
	{
		...
		CString strItem = _T("DefaultPath");

		GetPrivateProfileString(strItem, _T("Path"), _T(""),
	...
	void SaveDefaultPath(void)
	{
		...
		WritePrivateProfileString(strItem, _T("Path"), m_strDefaultPath, strPath);
	}
	void LoadRecentFile(void)
	{
		...
		const size_t nItems = m_vRecentFile.size();
