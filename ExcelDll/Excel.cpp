...
void CExcel::InputDataName()
{
	...
	{
		InputData2Excel(1, 1, _T("日期"), FALSE);
		InputData2Excel(1, 2, _T("時間"), FALSE);
		for (UINT nNameCouter = 0; nNameCouter < m_strDataName.size(); nNameCouter++)
		{
...
void CExcel::InputReportTime()
{
	if ((m_strReportTime.size() > 0) && (m_strReportTime.size() == 1))
	{
		InputData2Excel(m_nReportRowCounter, 1, m_strReportTime[0], FALSE);//采集时间
	}
	if ((m_strReportTime.size() > 0) && (m_strReportTime.size() == 2))
	{
		InputData2Excel(m_nReportRowCounter, 1, m_strReportTime[0], FALSE);//采集时间
		InputData2Excel(m_nReportRowCounter, 2, m_strReportTime[1], FALSE);//采集时间
	}
...
// 	CStdioFile	 logFile;
// 	CString path = _T("D:\\VC Project\\UDE_Report\\Back125（標準版本）\\Release\\Report\\123456_201805.xlsx");
